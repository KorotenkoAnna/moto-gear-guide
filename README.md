...
│   │   └── partials/       ← шапка сайта (с переключателем языка), футер, визуалы (фото + лого)
│   ├── _data/site.json     ← название сайта + меню/тексты по языку (ключи `fr`/`en`) —
│   │                          поменяете один раз и это применится везде
...
│   ├── en/                 ← английская версия сайта (см. раздел «Двуязычность» ниже) —
│   │                          зеркалит те же страницы, только под URL вида /en/...
...
✅ Вопрос FR/EN решён: сайт двуязычный. Восстановленные URL, у которых была английская версия,
сохранили её (файлы `*.en.njk`) — их трогать не нужно, там всё уже работает. Если позже решите
делать только FR — удалите файлы `*.en.njk` и настройте 301 с `/a/l/en/*` на FR-версию.

## Двуязычность (FR/EN)

- Французская версия — как раньше, без префикса в адресе: `/`, `/vestes-blousons/` и т.д.
- Английская версия — под префиксом `/en/`: `/en/`, `/en/jackets/`,
  `/en/helmets-gloves-protection/`, `/en/ballistic-protection/`, `/en/safety-standards/`,
  `/en/guides/`, `/en/about/`, `/en/contact/`. Файлы лежат в `src/en/` и повторяют структуру
  корневых French-файлов.
- Переключатель языка в шапке (`FR` / `EN`) берёт ссылку на «версию на другом языке» из поля
  `translationUrl:` в шапке файла (front matter). У каждой FR-страницы `translationUrl`
  указывает на её EN-версию и наоборот. Если у страницы нет `translationUrl` (например, у
  многих восстановленных legacy-страниц перевода просто не существует), переключатель ведёт на
  главную страницу другого языка.
- Название сайта («Ingénicité»), логотип и текст меню — общие компоненты в
  `src/_includes/partials/header.njk` и `src/_includes/partials/footer.njk`; сами тексты меню,
  теги `lang`/`locale` и т.п. для каждого языка — в `src/_data/site.json` (ключи `fr` и `en`).
...
Векторные SVG-иконки-заглушки заменены на настоящие фото со стока (Unsplash, лицензия
Unsplash License — свободное коммерческое использование без указания авторства). Файлы лежат
в `src/assets/img/photo-*.jpg`, макрос `icon()` в `src/_includes/partials/icons.njk` теперь
рендерит `<img>` вместо `<svg>` (сигнатура вызова та же — `{{ icon("jacket") }}` — поэтому
шаблоны почти не пришлось трогать).

Если захотите заменить какое-то фото на своё:
1. Кладёте новый файл в `src/assets/img/` (например, `photo-jacket.jpg`, перезаписав старый —
   имя должно совпадать, либо меняете путь в `icons.njk`).
2. Больше ничего менять не нужно — все картинки в `src/assets/img/` автоматически попадают в
   сборку.

Логотип в шапке — отдельно от фото: это векторная иконка шлема (SVG Repo, общественное
достояние / CC0), вставлена как inline-`<svg>` прямо в макрос `logo()` в `icons.njk` (не файл
в `assets/img/`), чтобы её можно было красить через CSS (`.brand-mark { color: ... }` в
`style.css`) — сейчас она в акцентном оранжевом цвете. Чтобы заменить логотип на свой, проще
всего отредактировать содержимое `<svg>` внутри макроса `logo()`.
...
- [x] Заменить SVG-иконки на настоящие фото
- [x] Согласовать вопрос FR/EN (двуязычность) — сайт двуязычный, FR на корне, EN под `/en/`
- [ ] Написать реальные тексты статей (в первую очередь — те, что в таблице выше, на обоих языках)
